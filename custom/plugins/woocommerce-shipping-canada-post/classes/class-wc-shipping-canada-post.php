    	...
    	$this->form_fields  = array(
			...
			'options' => array(
                    ...
                    'options' => array(
						...
						'PA18' => __( 'Proof of Age Required', 'wc_canada_post' ),
						'SO'   => __( 'Signature', 'wc_canada_post' )
					),
			...
			foreach ( $flat_packages as $flat_package ) {
				if ( isset( $this->lettermail_boxes[ $flat_package->id ] ) ) {
					...
					if ( isset( $this->options['COV'] ) && $package['destination']['country'] == 'CA' ) {
						$lettermail_cost += $this->ca_additional_liability_coverage * min( ceil( $this_box['value'] / 100 ), 50 );
					}
